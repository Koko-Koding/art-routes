...
function wp_art_routes_render_route_path_meta_box( $post ) {
	...
	// Instructions
	echo '<p>' . esc_html( __( 'Enter the route path as a series of latitude,longitude points. One point per line.', 'wp-art-routes' ) ) . '</p>';
	echo '<p>' . esc_html( __( 'Example: 52.3702, 4.8952', 'wp-art-routes' ) ) . '</p>';
	...
	echo '<p><button type="button" class="button" id="open_route_map">' . esc_html( __( 'Use Map to Create Route', 'wp-art-routes' ) ) . '</button></p>';
}
				...
				if ( ! empty( $artist_ids ) ) {
					foreach ( $artist_ids as $artist_id ) {
						...
						if ( $artist ) {
							...
							echo ' <a href="#" class="remove-artist">' . esc_html( __( 'Remove', 'wp-art-routes' ) ) . '</a>';
				...
				source: function(request, response) {
					...
					$.ajax({
						...
						data: {
							...
							term: request.term,
							post_type: postType,
							nonce: '<?php echo esc_attr( wp_create_nonce( 'artist_search_nonce' ) ); ?>'
						},
...
function wp_art_routes_render_artwork_icon_meta_box( $post ) {
	...
	<div id="artwork-icon-meta-box">
		...
		<select id="artwork_icon_select" name="artwork_icon" style="width: 100%;">
			<option value=""><?php esc_html_e( '-- Default (Art) --', 'wp-art-routes' ); ?></option>
	...
	<p class="description">
		<?php esc_html_e( 'Select a dashicon for this artwork. The icon will be displayed as a marker on the map.', 'wp-art-routes' ); ?>
	</p>
	<?php
}
...
function wp_art_routes_render_info_point_icon_meta_box( $post ) {
	...
	<div id="info-point-icon-meta-box">
		...
		<select id="info_point_icon_select" name="info_point_icon" style="width: 100%;">
			<option value=""><?php esc_html_e( '-- Default (Info) --', 'wp-art-routes' ); ?></option>
			<?php foreach ( $available_icons as $icon_class => $icon_label ) : ?>
				<option value="<?php echo esc_attr( $icon_class ); ?>" <?php selected( $selected_icon, $icon_class ); ?>>
					<?php echo esc_html( $icon_label ); ?>
				</option>
	...
	<p class="description">
		<?php esc_html_e( 'Select a dashicon for this information point. The icon will be displayed as a marker on the map.', 'wp-art-routes' ); ?>
	</p>
	<?php
}
