...
- Validate icon filenames against available icons list before saving to prevent storing arbitrary values
...
- **Dutch translations**: Added complete Dutch translations for Edition and Information Point labels in admin menu

### Changed

- **Admin menu order**: Reorganized menu to group entity types (Editions, Routes, Locations, Info Points) together at top, with utility pages (Dashboard, Import/Export, Settings) at bottom
...
- **WordPress Plugin Check**: Fixed all remaining errors (plugin now passes with 0 errors)
  - Fixed `__()` escaping → `esc_html__()` in multiple files
  - Fixed `wp_create_nonce`/`admin_url` escaping with `esc_js()`
  - Fixed `$container_style`, `$route_title`, `$route_types` escaping in templates
  - Replaced `move_uploaded_file()` with WP_Filesystem in settings.php
  - Replaced `fopen`/`fclose` with WP_Filesystem in import-export.php
  - Added ABSPATH check to art-route-map-template.php

### Changed

- **WordPress Version**: Updated "Tested up to" to WordPress 6.9

### Removed

- **Deprecated function**: Removed `load_plugin_textdomain()` (not needed since WordPress 4.6)
...
- **WordPress.org Compliance**: Bundled Leaflet.js 1.9.4 locally instead of loading from CDN (required for WordPress.org plugin directory)
...
- **Template System**: Implemented proper template handling that allows theme overrides
  - Template can be customized by copying to theme's `art-routes/single-artwork.php`
