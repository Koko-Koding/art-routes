...
![Art Routes Logo](./assets/plugin-icons/icon.webp)
...
Art Routes is a flexible WordPress plugin for organizations managing cultural location-based events (art routes, theater trails, music festivals, heritage walks, etc.). It provides interactive map-based route management using Leaflet.js and OpenStreetMap.
...
1. Upload the plugin files to the `/wp-content/plugins/art-routes` directory, or install the plugin through the WordPress plugins screen.
...
The artwork template can be customized by themes by copying `templates/single-artwork.php` to `art-routes/single-artwork.php` in your active theme directory.
...
1. Copy the `art-routes.pot` file from the `languages` folder
2. Rename it to `art-routes-{language_code}.po` (e.g., `art-routes-fr_FR.po` for French)
...
wp i18n make-pot . languages/art-routes.pot

# To merge the .pot file with existing translations
msgmerge --update languages/art-routes-nl_NL.po languages/art-routes.pot

# Generate the .po file for Dutch
wp i18n make-po languages/art-routes.pot languages/art-routes-nl_NL.po

# Generate the .mo file for Dutch
wp i18n make-mo languages/art-routes-nl_NL.po
```
