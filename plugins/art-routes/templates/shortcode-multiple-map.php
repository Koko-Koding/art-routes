...
// Variables should be passed via art_routes_get_template_part()
// $atts = shortcode attributes
// $routes = array of route data

// Check if routes data exists
if (empty($routes)) {
    echo '<p class="art-routes-error">' . esc_html__('No routes found.', 'art-routes') . '</p>';
    return;
}
...
$js_data = [
    ...
    'routes' => [],
    'artworks' => art_routes_get_all_artworks(), // Add global artworks
    'information_points' => art_routes_get_all_information_points(), // Add global info points
    'colors' => $route_colors,
    'mapSettings' => [
...
wp_enqueue_script('art-routes-multiple-routes-map-js');

// Pass per-instance data via inline script
wp_add_inline_script('art-routes-multiple-routes-map-js',
    'var artRoutesMultiMapInstances = artRoutesMultiMapInstances || [];' .
    'artRoutesMultiMapInstances.push(' . wp_json_encode([
        'mapId'   => $map_id,
        'mapData' => $js_data,
    ]) . ');',
    'before'
