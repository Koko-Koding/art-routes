 ...
 */
function art_routes_ajax_mark_artwork_visited()
{
    // Verify nonce
    if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'art_routes_nonce')) {
    ...
    $visited = get_user_meta($user_id, 'art_routes_visited_artworks', true);
    ...
    if (!in_array($artwork_id, $visited)) {
        $visited[] = $artwork_id;
        update_user_meta($user_id, 'art_routes_visited_artworks', $visited);
    }
...
add_action('wp_ajax_art_routes_mark_artwork_visited', 'art_routes_ajax_mark_artwork_visited');
add_action('wp_ajax_nopriv_art_routes_mark_artwork_visited', 'art_routes_ajax_mark_artwork_visited');
...
function wp_ajax_get_route_points()
{
    ...
    $points = art_routes_get_associated_points($route_id);
 ...
 */
function art_routes_get_associated_points($route_id)
{
    $points = [
        'artworks' => [],
        'information_points' => [],
    ...
    foreach ($artwork_posts as $artwork_post) {
        ...
        if (is_numeric($latitude) && is_numeric($longitude)) {
            ...
            if (!empty($icon_filename)) {
                ...
                $icon_url = art_routes_get_icon_url($icon_filename);
            } else {
                ...
                if (!empty($default_location_icon)) {
                    $icon_url = art_routes_get_icon_url($default_location_icon);
                }
            }
    ...
    foreach ($info_point_posts as $info_post) {
        ...
        if (!empty($icon_filename)) {
            ...
            if (empty($icon_url)) {
                $icon_url = art_routes_get_icon_url(art_routes_get_default_info_icon());
            }
        }
 ...
 */
function art_routes_ajax_export_gpx()
{
    // Verify nonce - for GET requests, the nonce is in the URL parameter
    $nonce = isset($_GET['_wpnonce']) ? sanitize_text_field(wp_unslash($_GET['_wpnonce'])) : '';
    if (!wp_verify_nonce($nonce, 'art_routes_export_gpx')) {
    ...
    $gpx_content = art_routes_generate_gpx($route_data);
    ...
    // Note: $gpx_content is pre-escaped XML generated by art_routes_generate_gpx()
    // using art_routes_sanitize_for_gpx() which applies htmlspecialchars() with ENT_XML1
    // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- GPX/XML content already escaped for XML output
    echo $gpx_content;
    exit;
}
add_action('wp_ajax_art_routes_export_gpx', 'art_routes_ajax_export_gpx');
add_action('wp_ajax_nopriv_art_routes_export_gpx', 'art_routes_ajax_export_gpx');
 ...
 */
function art_routes_sanitize_for_gpx($content) {
    if (empty($content)) {
        return '';
    }
 ...
 *
 * @param array $route_data Route data from art_routes_get_route_data()
 * @return string GPX XML content
 */
function art_routes_generate_gpx($route_data)
{
    $route_title = art_routes_sanitize_for_gpx($route_data['title']);
    $route_description = art_routes_sanitize_for_gpx($route_data['description']);
    ...
    if (!empty($route_data['artworks'])) {
        foreach ($route_data['artworks'] as $index => $artwork) {
            $artwork_name = art_routes_sanitize_for_gpx($artwork['title']);
            $artwork_desc = art_routes_sanitize_for_gpx($artwork['description'] ?? $artwork['excerpt'] ?? '');
            $artwork_number = !empty($artwork['number']) ? art_routes_sanitize_for_gpx($artwork['number']) : ($index + 1);
    ...
    if (!empty($route_data['information_points'])) {
        foreach ($route_data['information_points'] as $index => $info_point) {
            $info_name = art_routes_sanitize_for_gpx($info_point['title']);
            $info_desc = art_routes_sanitize_for_gpx($info_point['description'] ?? $info_point['excerpt'] ?? '');
