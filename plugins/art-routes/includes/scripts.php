 ...
 */
function art_routes_register_frontend_assets()
{
    // Leaflet CSS (bundled locally for WordPress.org compliance)
    wp_register_style(
        'art-routes-leaflet-css',
        ART_ROUTES_PLUGIN_URL . 'assets/lib/leaflet/leaflet.css',
        [],
        '1.9.4'
    );
    ...
    wp_register_script(
        'art-routes-leaflet-js',
        ART_ROUTES_PLUGIN_URL . 'assets/lib/leaflet/leaflet.js',
        [],
        '1.9.4',
        true
    );
    ...
    wp_register_style(
        'art-routes-map-css',
        ART_ROUTES_PLUGIN_URL . 'assets/css/art-route-map.css',
        [],
        ART_ROUTES_VERSION
    );
    ...
    wp_register_style(
        'art-routes-related-artworks-css',
        ART_ROUTES_PLUGIN_URL . 'assets/css/related-artworks.css',
        [],
        ART_ROUTES_VERSION
    );
    ...
    wp_register_style(
        'art-routes-edition-map-shortcode-css',
        ART_ROUTES_PLUGIN_URL . 'assets/css/edition-map-shortcode.css',
        ['art-routes-leaflet-css'],
        ART_ROUTES_VERSION
    );
    ...
    // Single edition CSS
    wp_register_style(
        'art-routes-single-edition-css',
        ART_ROUTES_PLUGIN_URL . 'assets/css/single-edition.css',
        ['art-routes-leaflet-css'],
        ART_ROUTES_VERSION
    );
    ...
    // Single artwork CSS
    wp_register_style(
        'art-routes-single-artwork-css',
        ART_ROUTES_PLUGIN_URL . 'assets/css/single-artwork.css',
        ['art-routes-leaflet-css'],
        ART_ROUTES_VERSION
    );
    ...
    // Route icons CSS
    wp_register_style(
        'art-routes-route-icons-css',
        ART_ROUTES_PLUGIN_URL . 'assets/css/route-icons.css',
        [],
        ART_ROUTES_VERSION
    );
    ...
    wp_register_script(
        'art-routes-edition-map-js',
        ART_ROUTES_PLUGIN_URL . 'assets/js/edition-map.js',
        ['art-routes-leaflet-js'],
        ART_ROUTES_VERSION,
        true
    );
    ...
    wp_localize_script('art-routes-edition-map-js', 'artRoutesEditionMapI18n', [
    ...
    wp_register_script(
        'art-routes-multiple-routes-map-js',
        ART_ROUTES_PLUGIN_URL . 'assets/js/multiple-routes-map.js',
        ['art-routes-leaflet-js'],
        ART_ROUTES_VERSION,
        true
    );
    ...
    wp_localize_script('art-routes-multiple-routes-map-js', 'artRoutesMultiMapI18n', [
    ...
    wp_register_script(
        'art-routes-single-artwork-map-js',
        ART_ROUTES_PLUGIN_URL . 'assets/js/single-artwork-map.js',
        ['jquery', 'art-routes-leaflet-js'],
        ART_ROUTES_VERSION,
        true
    );
    ...
    // Route icons JS
    wp_register_script(
        'art-routes-route-icons-js',
        ART_ROUTES_PLUGIN_URL . 'assets/js/route-icons.js',
        [],
        ART_ROUTES_VERSION,
        true
    );
    ...
    wp_register_script(
        'art-routes-map-js',
        ART_ROUTES_PLUGIN_URL . 'assets/js/art-route-map.js',
        ['jquery', 'art-routes-leaflet-js'],
        ART_ROUTES_VERSION,
        true
    );
}
add_action('wp_enqueue_scripts', 'art_routes_register_frontend_assets', 5);

/**
 * Enqueue frontend scripts and styles for the art route map
 */
function art_routes_enqueue_scripts()
{
    ...
    // Enqueue Leaflet
    wp_enqueue_style('art-routes-leaflet-css');
    wp_enqueue_script('art-routes-leaflet-js');

    // Our custom CSS
    wp_enqueue_style('art-routes-map-css');

    // Related artworks CSS (only if shortcode is present)
    if ($has_related_artworks_shortcode) {
        wp_enqueue_style('art-routes-related-artworks-css');
    }
    ...
    // Our custom JS
    wp_enqueue_script('art-routes-map-js');
    ...
    if (is_singular('art_route')) {
        ...
        if (isset($post->post_content) && has_shortcode($post->post_content, 'art_route_map')) {
            ...
            if (!empty($matches[2])) {
                ...
                $route_id = get_option('art_routes_default_route', 0);
            }
        }
    }
    ...
    if ($route_data) {
        $js_data = [
            ...
            'nonce' => wp_create_nonce('art_routes_nonce'),
...
add_action('wp_enqueue_scripts', 'art_routes_enqueue_scripts');

/**
 * Enqueue admin scripts and styles for the route editor, location picker, and other admin pages
 */
function art_routes_enqueue_admin_scripts($hook)
{
    global $post;
    ...
    if ($hook === 'edition_page_art-routes-settings') {
        ...
        wp_localize_script('art-routes-custom-icons-js', 'artRoutesCustomIcons', [
            'deleteNonce' => wp_create_nonce('art_routes_delete_icon'),
            'i18n' => [
    ...
    if ($hook === 'edition_page_art-routes-import-export') {
        ...
        wp_enqueue_script(
            'art-routes-import-export-js',
            ART_ROUTES_PLUGIN_URL . 'assets/js/admin/import-export.js',
            [],
            ART_ROUTES_VERSION,
            true
        );
        wp_localize_script('art-routes-import-export-js', 'artRoutesImportExport', [
    ...
    wp_enqueue_style(
        'art-routes-meta-boxes-css',
        ART_ROUTES_PLUGIN_URL . 'assets/css/meta-boxes.css',
        [],
        ART_ROUTES_VERSION
    );
    ...
    wp_enqueue_script(
        'art-routes-icon-preview-js',
        ART_ROUTES_PLUGIN_URL . 'assets/js/admin/icon-preview.js',
        ['jquery'],
        ART_ROUTES_VERSION,
        true
    );
    ...
    if ($is_route_type) {
        wp_enqueue_style(
            'art-routes-editor-css',
            ART_ROUTES_PLUGIN_URL . 'assets/css/route-editor-admin.css',
            [],
            ART_ROUTES_VERSION
        );
        ...
        wp_enqueue_script(
            ...
            ART_ROUTES_PLUGIN_URL . 'assets/js/route-editor-admin.js',
            ['jquery', 'art-routes-admin-leaflet-js', 'jquery-ui-draggable'],
            ART_ROUTES_VERSION,
            true
        );
    ...
    if ($is_artwork_type || $is_info_point_type) {
        wp_enqueue_style(
            'art-routes-location-picker-css',
            ART_ROUTES_PLUGIN_URL . 'assets/css/artwork-location-picker.css',
            [],
            ART_ROUTES_VERSION
        );
        ...
        wp_enqueue_script(
            ...
            ART_ROUTES_PLUGIN_URL . 'assets/js/artwork-location-picker.js',
            ['jquery', 'art-routes-admin-leaflet-js'],
            ART_ROUTES_VERSION,
            true
        );
        ...
        wp_enqueue_script(
            'art-routes-location-map-mini-js',
            ART_ROUTES_PLUGIN_URL . 'assets/js/admin/location-map-mini.js',
            ['jquery', 'art-routes-admin-leaflet-js'],
            ART_ROUTES_VERSION,
            true
        );
        ...
        if ($is_artwork_type) {
            ...
            wp_enqueue_script(
                'art-routes-artist-search-js',
                ART_ROUTES_PLUGIN_URL . 'assets/js/admin/artist-search.js',
...
add_action('admin_enqueue_scripts', 'art_routes_enqueue_admin_scripts');

/**
 * Check if current page should display a route map
 */
function art_routes_is_route_page()
{
    global $post;
    ...
    if (isset($post->post_content)) {
        ...
        if (has_block('art-routes/edition-map', $post->post_content)) {
            return true;
        }
        if (has_block('art-routes/routes-map', $post->post_content)) {
            return true;
        }
    }
 ...
 */
function art_routes_get_route_editor_modal_html()
{
    ob_start();
?>
 ...
 */
function art_routes_get_location_picker_modal_html()
{
    ob_start();
?>
