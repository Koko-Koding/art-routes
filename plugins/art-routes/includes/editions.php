<?php
/**
 * Edition Custom Post Type for Art Routes Plugin
 *
...
add_action('init', 'art_routes_register_edition_post_type');
...
function art_routes_register_edition_meta()
{
    ...
    register_post_meta('edition', '_edition_terminology', [
        ...
        'sanitize_callback' => 'art_routes_sanitize_edition_terminology',
...
add_action('init', 'art_routes_register_edition_meta');
 ...
 */
function art_routes_sanitize_edition_terminology($value)
{
    if (!is_array($value)) {
        return [];
    }
 ...
 */
function art_routes_add_edition_meta_boxes()
{
...
add_action('add_meta_boxes', 'art_routes_add_edition_meta_boxes');
 ...
 */
function art_routes_render_edition_terminology_meta_box($post)
{
 ...
 * @param WP_Post $post The post object
 */
function art_routes_render_edition_dates_meta_box($post)
{
    // Add nonce for security
 ...
 * @param WP_Post $post The post object
 */
function art_routes_render_edition_settings_meta_box($post)
{
    // Add nonce for security
    ...
    $global_default = get_option('art_routes_default_location_icon', '');
    $global_label = $global_default ? art_routes_get_icon_display_name($global_default) : __('none', 'art-routes');
    ...
    <?php if (!empty($default_icon)) : ?>
        <p style="text-align: center;">
            <img src="<?php echo esc_url(art_routes_get_icon_url($default_icon)); ?>" alt="" style="width: 48px; height: 48px;">
        </p>
    <?php endif; ?>
 ...
 */
function art_routes_save_edition_settings($post_id)
{
    // Verify nonce
 ...
 * @param int $post_id The post ID
 */
function art_routes_save_edition_terminology($post_id)
{
    // Verify nonce
 ...
 * @param int $post_id The post ID
 */
function art_routes_save_edition_dates($post_id)
{
    // Verify nonce
 ...
 */
function art_routes_get_edition_data($edition_id)
{
 ...
 */
function art_routes_get_post_edition($post_id)
{
 ...
 */
function art_routes_edition_label($type, $plural = false, $edition_id = null)
{
    return art_routes_label($type, $plural, $edition_id);
}
 ...
 */
function art_routes_enqueue_edition_delete_modal_assets($hook) {
    ...
    // Enqueue our CSS
    wp_enqueue_style(
        'art-routes-edition-delete-modal',
    ...
    // Localize script
    wp_localize_script('art-routes-edition-delete-modal', 'artRoutesEditionDelete', [
        'ajaxUrl' => admin_url('admin-ajax.php'),
        'nonce' => wp_create_nonce('art_routes_edition_delete'),
        'strings' => [
...
add_action('admin_enqueue_scripts', 'art_routes_enqueue_edition_delete_modal_assets');

/**
 * AJAX handler to get content counts for editions
 * Used by the delete confirmation modal
 */
function art_routes_ajax_get_edition_content_counts() {
    check_ajax_referer('art_routes_edition_delete', 'nonce');
...
add_action('wp_ajax_art_routes_get_edition_content_counts', 'art_routes_ajax_get_edition_content_counts');

/**
 * AJAX handler to delete editions only (keep linked content)
 * Clears _edition_id meta from linked content before deletion
 */
function art_routes_ajax_delete_edition_only() {
    check_ajax_referer('art_routes_edition_delete', 'nonce');
...
add_action('wp_ajax_art_routes_delete_edition_only', 'art_routes_ajax_delete_edition_only');
 ...
 */
function art_routes_ajax_delete_edition_all() {
    check_ajax_referer('art_routes_edition_delete', 'nonce');
...
add_action('wp_ajax_art_routes_delete_edition_all', 'art_routes_ajax_delete_edition_all');
