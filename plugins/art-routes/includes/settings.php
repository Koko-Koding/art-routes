<?php
/**
 * Settings Page for Art Routes Plugin
 *
 ...
 */
function art_routes_sanitize_icon_filename($input) {
    if (empty($input)) {
        return '';
    }
 ...
 */
function art_routes_sanitize_terminology($input) {
    if (!is_array($input)) {
        return [];
    }
 ...
 */
function art_routes_register_settings() {
    // General settings
    register_setting(
        'art_routes_options',
        'art_routes_default_route_id',
        [
            'type' => 'integer',
    ...
    register_setting(
        'art_routes_options',
        'art_routes_enable_location_tracking',
        [
            'type' => 'boolean',
    ...
    register_setting(
        'art_routes_options',
        'art_routes_default_location_icon',
        [
            'type' => 'string',
            'sanitize_callback' => 'art_routes_sanitize_icon_filename',
            'default' => '',
        ]
    );
    ...
    register_setting(
        'art_routes_terminology_options',
        'art_routes_terminology',
        [
            'type' => 'array',
            'sanitize_callback' => 'art_routes_sanitize_terminology',
            'default' => [],
        ]
    );
}
add_action('admin_init', 'art_routes_register_settings');
...
function art_routes_add_settings_page() {
    add_submenu_page(
        ...
        'manage_options',
        'art-routes-settings',
        'art_routes_render_settings_page',
        101 // Position: last in menu
    );
}
add_action('admin_menu', 'art_routes_add_settings_page');

/**
 * Render the settings page with tabs
 */
function art_routes_render_settings_page() {
    ...
    <div class="wrap">
        ...
        <?php settings_errors('art_routes_messages'); ?>
        ...
        <div class="tab-content" style="margin-top: 20px;">
            ...
            switch ($current_tab) {
                case 'terminology':
                    art_routes_render_terminology_tab();
                    break;
                case 'custom_icons':
                    art_routes_render_custom_icons_tab();
                    break;
                case 'general':
                default:
                    art_routes_render_general_tab();
                    break;
            }
            ?>
        </div>
 ...
 */
function art_routes_render_general_tab() {
    ?>
    <form method="post" action="options.php">
        <?php settings_fields('art_routes_options'); ?>

        <table class="form-table" role="presentation">
            <tr>
                <th scope="row">
                    <label for="art_routes_default_route_id">
                        <?php esc_html_e('Default Route', 'art-routes'); ?>
                    </label>
                </th>
                <td>
                    <?php
                    $default_route_id = get_option('art_routes_default_route_id', 0);
                    ...
                    if (!empty($routes)) {
                        echo '<select name="art_routes_default_route_id" id="art_routes_default_route_id">';
            ...
            <tr>
                ...
                <td>
                    <label for="art_routes_enable_location_tracking">
                        <input type="checkbox" name="art_routes_enable_location_tracking" id="art_routes_enable_location_tracking" value="1" <?php checked(get_option('art_routes_enable_location_tracking', true)); ?> />
            ...
            <tr>
                <th scope="row">
                    <label for="art_routes_default_location_icon">
                        <?php esc_html_e('Default Location Icon', 'art-routes'); ?>
                    </label>
                </th>
                <td>
                    <?php
                    $default_location_icon = get_option('art_routes_default_location_icon', '');
                    $available_icons = art_routes_get_available_icons();
                    ?>
                    <select name="art_routes_default_location_icon" id="art_routes_default_location_icon">
                        ...
                        <?php foreach ($available_icons as $icon_filename) : ?>
                            <option value="<?php echo esc_attr($icon_filename); ?>" <?php selected($default_location_icon, $icon_filename); ?>>
                                <?php echo esc_html(art_routes_get_icon_display_name($icon_filename)); ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                    <?php if (!empty($default_location_icon)) : ?>
                        <span style="margin-left: 10px; vertical-align: middle;">
                            <img src="<?php echo esc_url(art_routes_get_icon_url($default_location_icon)); ?>" alt="" style="width: 24px; height: 24px; vertical-align: middle;">
                        </span>
                    <?php endif; ?>
                    <p class="description">
 ...
 */
function art_routes_render_terminology_tab() {
    ...
    ?>
    <form method="post" action="options.php">
        <?php settings_fields('art_routes_terminology_options'); ?>
        ...
        <?php foreach ($terminology_types as $type => $config) : ?>
            ...
            <table class="form-table" role="presentation">
                <tr>
                    <th scope="row">
                        <label for="art_routes_terminology_<?php echo esc_attr($type); ?>_singular">
                            <?php esc_html_e('Singular', 'art-routes'); ?>
                        </label>
                    </th>
                    <td>
                        <input type="text"
                               name="art_routes_terminology[<?php echo esc_attr($type); ?>][singular]"
                               id="art_routes_terminology_<?php echo esc_attr($type); ?>_singular"
                ...
                <tr>
                    <th scope="row">
                        <label for="art_routes_terminology_<?php echo esc_attr($type); ?>_plural">
                            <?php esc_html_e('Plural', 'art-routes'); ?>
                        </label>
                    </th>
                    <td>
                        <input type="text"
                               name="art_routes_terminology[<?php echo esc_attr($type); ?>][plural]"
                               id="art_routes_terminology_<?php echo esc_attr($type); ?>_plural"
                ...
                <?php if ($config['has_slug']) : ?>
                    <tr>
                        <th scope="row">
                            <label for="art_routes_terminology_<?php echo esc_attr($type); ?>_slug">
                                <?php esc_html_e('URL Slug', 'art-routes'); ?>
                            </label>
                        </th>
                        <td>
                            <input type="text"
                                   name="art_routes_terminology[<?php echo esc_attr($type); ?>][slug]"
                                   id="art_routes_terminology_<?php echo esc_attr($type); ?>_slug"
 ...
 */
function art_routes_render_custom_icons_tab() {
    $custom_icons = art_routes_get_custom_icons();
    $custom_icons_url = art_routes_get_custom_icons_url();
    ?>
    <div class="custom-icons-wrapper">
        ...
        <div class="custom-icons-upload-form" style="margin: 20px 0; padding: 20px; background: #f9f9f9; border: 1px solid #ddd; border-radius: 4px;">
            <form id="custom-icon-upload-form" enctype="multipart/form-data">
                <?php wp_nonce_field('art_routes_upload_icon', 'upload_icon_nonce'); ?>
                <p>
 ...
 */
function art_routes_ajax_upload_custom_icon() {
    // Verify nonce
    if (!isset($_POST['upload_icon_nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['upload_icon_nonce'])), 'art_routes_upload_icon')) {
    ...
    $custom_icons_dir = art_routes_get_custom_icons_dir();
    if (!$custom_icons_dir) {
    ...
    wp_send_json_success([
        ...
        'filename' => $filename,
        'url' => art_routes_get_custom_icons_url() . rawurlencode($filename),
    ]);
}
add_action('wp_ajax_art_routes_upload_custom_icon', 'art_routes_ajax_upload_custom_icon');

/**
 * Handle custom icon deletion via AJAX
 */
function art_routes_ajax_delete_custom_icon() {
    // Verify nonce
    if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'art_routes_delete_icon')) {
        wp_send_json_error(['message' => __('Security check failed.', 'art-routes')]);
    }
    ...
    if (!art_routes_is_custom_icon($filename)) {
...
add_action('wp_ajax_art_routes_delete_custom_icon', 'art_routes_ajax_delete_custom_icon');
