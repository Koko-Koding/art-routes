 ...
 * Expects wpArtRoutesArtistSearch to be localized with: { nonce, removeText }
 *
 * @package WP Art Routes
 */

jQuery(document).ready(function($) {
    if (typeof wpArtRoutesArtistSearch === 'undefined') {
        return;
    }
        ...
        source: function(request, response) {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    term: request.term,
                    post_type: postType,
                    nonce: wpArtRoutesArtistSearch.nonce
                },
    ...
    function addArtistToList(item) {
        ...
        if ($('#selected_artists_list li[data-id="' + item.id + '"]').length === 0) {
            ...
            artistItem.append(' <a href="#" class="remove-artist">' + wpArtRoutesArtistSearch.removeText + '</a>');
